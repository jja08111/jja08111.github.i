...
$ F_0=1, \ \ F_1=1 \\\\  
F_{n+2}=F_{n+1}+F_n \ \ (n=0,1,2 \cdots) $
...
그러면 $ x_n=Ax_{n-1} $ 이므로 아래와 같은 접근이 가능하다. 

$ x_n=Ax_{n-1}=A(Ax_{n-2})=A(A(Ax_{n-3}))= \cdots = A^{n-1}x_1 $
