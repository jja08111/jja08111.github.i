...
그 이유는 다음과 같다. 
1. 먼저 그래프에서 $1$번 정점을 방문하고 우선순위 큐에 $2,3$번 정점을 추가한다.  
...
2. 다시 큐에 들어있던 $3$번 정점을 꺼내어 방문하려는데 이미 $dist[3]$에는 $2$가 저장되어 있어 방문하지 못한다.  

따라서 $dist$배열을 각 정점에 대한 거리를 현재 사용한 비용별로, 즉 2차원으로 구현해야 한다. 
